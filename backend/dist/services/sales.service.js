...
class SalesService {
    ...
    static async createSaleWithInventoryUpdate(saleData) {
        ...
        try {
            const saleRepo = queryRunner.manager.getRepository(Sale_1.Sale);
            // Create sale
            const sale = saleRepo.create(saleData);
            await queryRunner.manager.save(sale);
            // Update inventory based on sale type
            if (saleData.tipo === "producto") {
                const prodRepo = queryRunner.manager.getRepository(Product_1.Product);
                const product = await prodRepo.findOne({
                    where: { id: saleData.referenciaId },
                });
                if (!product) {
                    throw ApiError_1.ApiError.notFound(`Product not found: ${saleData.referenciaId}`);
                }
                ...
                if (product.tipo === "huevo" && product.cantidadHuevos) {
                    if (product.cantidadHuevos < saleData.cantidad) {
                        throw ApiError_1.ApiError.badRequest(`Insufficient egg quantity. Available: ${product.cantidadHuevos}, Requested: ${saleData.cantidad}`);
                    }
                ...
                if (!animal) {
                    throw ApiError_1.ApiError.notFound(`Animal not found: ${saleData.referenciaId}`);
                }
                if (animal.cantidad < saleData.cantidad) {
                    throw ApiError_1.ApiError.badRequest(`Insufficient animal quantity. Available: ${animal.cantidad}, Requested: ${saleData.cantidad}`);
                }
